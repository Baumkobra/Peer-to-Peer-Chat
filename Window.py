...
class Window(Frame):
    def __init__(self, master=None):
        ...
        self.text_btn = Button(
            frame1, height=1, width=10, text="send", command=self.get_text_input
        )
    ...
    def send_file(self):
        ...
        if not self.has_socket:
            self.display(
                data="Establish a connection before sending a file\n", user="[INFO]"
            )
            return
    ...
    def on_file_receive(self, **kwargs):
        ...
        self.display(
            data=f"new file received:{kwargs['file_name']}.{kwargs['file_extension']}, {kwargs['file_size']}bytes\n",
            user="[INFO]",
        )
    ...
    def get_file(self):
        ...
        def get():
            ...
            for name, var in available_files.items():
                ...
                if var:
                    ...
                    self.display(
                        data=f"Datei {file['file_name']} in {info} gespeichert.\n",
                        user="[INFO]",
                    )
                else:
        ...
        for file in files.values():
            ...
            Checkbutton(
                top, text=f, variable=bvar, onvalue=True, offvalue=False, anchor="w"
            ).pack(anchor="w")
    ...
    def display(self, **kwargs):
        text = kwargs["data"]
        user = (
            kwargs["user"] if type(kwargs["user"]) is str else kwargs["user"].decode()
        )
    ...
    def send(self, text):
        if not self.has_socket:
            self.display(
                data="You are not currently connected or hosting a connection\n",
                user="[INFO]",
            )
            return
        if not self.is_connected:
            self.display(data="You are not yet connected\n", user="[INFO]")
            return
    ...
    def host(self):
        if self.has_socket:
            self.display(
                data="can't create a new socket, close the old one first\n",
                user="[INFO]",
            )
            return
        ...
        def do():
            self.sock = Server(
                port=50000,
                onmessage_callback=self.display,
                onconnection_close=self.on_connection_close,
                onconnection_open=self.on_connection_open,
                onfile_receive=self.on_file_receive,
            )
            self.display(
                data=f"attemting to host socket on {self.sock.host}:{self.sock.port}\n",
                user="[INFO]",
            )
            ...
            self.display(
                data=f"hosting chat on {self.sock.host}:{self.sock.port}\n",
                user="[INFO]",
            )
    ...
    def connect(self):
        if self.has_socket:
            self.display(
                data="can't a new socket, close the old one first\n", user="[INFO]"
            )
            return
        ...
        def do():
            ...
            self.display(
                data=f"attempting to connect to {self.sock.host if type(self.sock.host) is str else self.sock.host.decode()}:{self.sock.port}\n",
                user="[INFO]",
            )
            ...
            self.display(
                data=f"joining chat on {self.sock.host if type(self.sock.host) is str else self.sock.host.decode()}:{self.sock.port}\n",
                user="[INFO]",
            )
