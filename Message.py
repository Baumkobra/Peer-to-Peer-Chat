...
def read_message(message_bytes: bytes) -> dict[str:bytes]:
    ...
    if len(message_bytes.split(SEPERATOR)) > 11:
        debug("seperator error")
    elif len(message_bytes.split(SEPERATOR)) < 11:
        debug("seperator error")
...
def format_message(
    ...
    for key, item in bufferdict.items():
        ...
        currentbuffer = key
        amount_of_buffers = (
            amount_of_buffers.__str__().encode()
            if type(amount_of_buffers) is int
            else amount_of_buffers
        )
...
def is_encrypted(message_bytes):
    if STARTER and SEPERATOR and ENDER in message_bytes:
        debug("is not encrypted")
        return False
    debug("is encrypted")
    return True
...
def is_last_buffer(buffer: bytes, amount_of_buffers: bytes) -> bool:
    if buffer == amount_of_buffers:
        return True
    return False


def is_complete_message(
    bufferdict, amount_of_buffers: bytes, list_of_buffers: list
) -> bool:
